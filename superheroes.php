...
if (isset($_GET['query'])) {
    ...
    foreach ($superheroes as $superhero) {
        if (
            stripos($superhero['name'], $query) !== false ||
            stripos($superhero['alias'], $query) !== false
        ) {
            echo '<h3>' . strtoupper(htmlspecialchars($superhero['alias'], ENT_QUOTES, 'UTF-8')) . '</h3>';
    ...
    if (! $found) {
        echo '<p class="errorMessage">SUPERHERO NOT FOUND</p>';
    }
}
